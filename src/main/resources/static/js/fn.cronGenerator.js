/*
@author https://www.freeformatter.com/cron-expression-generator-quartz.html
*/
var FF = {
	cron2text: function () {
		...
		$.ajax({
			...
			success: function (data) {
				if (data && data.description) {
					_notifications(data.description, 'info');
				} else {
					_notifications(data.error, 'error');
				}
			}
		});
	},
	cron: function () {
		...
		$('#cronResult').val(seconds + ' ' + minutes + ' ' + hours + ' ' + dom + ' ' + month + ' ' + dow + ' ' + year);
	},
	seconds: function () {
		var seconds = '';
